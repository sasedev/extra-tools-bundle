...
namespace Sasedev\ExtraToolsBundle\Command;
...
class UpdateTransCommand extends ContainerAwareCommand
{
	...
	{
		$this->setName('sasedev:trans:update')
			->setDescription('Update the translation file')
			->setDefinition(
			array(new InputArgument('locale', InputArgument::REQUIRED, 'The locale'), 
				new InputArgument('bundle', InputArgument::REQUIRED, 'The bundle where to load the messages'), 
				new InputOption('prefix', null, InputOption::VALUE_OPTIONAL, 'Override the default prefix', '__'), 
				new InputOption('output-format', null, InputOption::VALUE_OPTIONAL, 
					'Override the default output format (yml, xliff, php or pot)', 'yml'), 
				new InputOption('source-lang', null, InputOption::VALUE_OPTIONAL, 'Set the source language attribute in xliff files', 'en'), 
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		if ($input->getOption('force') !== true && $input->getOption('dump-messages') !== true) {
			...
			$foundBundle = $this->getApplication()
				->getKernel()
				->getBundle($input->getArgument('bundle'));
			$bundleTransPath = $foundBundle->getPath() . '/Resources/translations';
			$output->writeln(
				sprintf('Generating "<info>%s</info>" translation files for "<info>%s</info>"', $input->getArgument('locale'), 
					$foundBundle->getName()));
			...
			$files = $finder->files()
				->name('*.html.twig')
				->in($foundBundle->getPath() . '/Resources/views/');
			...
			$finder = new Finder();
			$files = $finder->files()
				->name('*.' . $input->getArgument('locale') . '.yml')
				->in($bundleTransPath);
			foreach ($files as $file) {
			...
			$finder = new Finder();
			$files = $finder->files()
				->name('*.' . $input->getArgument('locale') . '.xliff')
				->in($bundleTransPath);
			foreach ($files as $file) {
				$output->writeln(sprintf(' > parsing translation <comment>%s</comment>', $file->getPathname()));
			...
			$finder = new Finder();
			$files = $finder->files()
				->name('*.' . $input->getArgument('locale') . '.php')
				->in($bundleTransPath);
			foreach ($files as $file) {
				$output->writeln(sprintf(' > parsing translation <comment>%s</comment>', $file->getPathname()));
			...
			$finder = new Finder();
			$files = $finder->files()
				->name('*.' . $input->getArgument('locale') . '.pot')
				->in($bundleTransPath);
			foreach ($files as $file) {
				$output->writeln(sprintf(' > parsing translation <comment>%s</comment>', $file->getPathname()));
